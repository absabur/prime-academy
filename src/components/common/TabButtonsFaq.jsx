...
const TabButtonsFaq = ({ data, selected, setSelected }) => {
  return (
    <div>
      {/* Desktop tabs */}
        ...
        {data?.length &&
          data?.map((item, index) => (
            <button
              ...
              onClick={() => setSelected(item.faq_nav_slug)}
            >
              {item.faq_nav}
            </button>
          ))}
      ...
      <div className="md:hidden mb-4">
        ...
        >
          {data?.length &&
            data?.map((item, index) => (
              <option key={index} value={item.faq_nav_slug}>
                {item.faq_nav}
              </option>
            ))}
        </select>
